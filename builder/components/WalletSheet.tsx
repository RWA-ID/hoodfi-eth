...
import { REBUILD_USD } from "@/lib/labels";

/*
 * "Where is my wallet, and how do I put money in it?"
 *
 * Signing in with Google hands someone a non-custodial wallet and no idea what to
 * do next. AppKit's own account view is not the answer here: for an embedded
 * wallet it renders blank after any reload (reown-com/appkit#5765), which is
 * exactly the state someone is in when they come back to fund it before
 * republishing.
 *
 ...
 *    nothing at all, including a republish priced in USDG — so every route here
 *    lands ETH, not a stablecoin, even though this app accepts both.
 *  - You cannot withdraw from the Robinhood BROKERAGE app onto Robinhood Chain.
 *    Robinhood's own docs say to bridge. The app that can send here is Robinhood
 *    WALLET, the separate self-custody one.
 *  - MoonPay does sell ETH on Robinhood Chain directly (confirmed in the app
 *    2026-09-05). Press coverage of the integration mentions only USDG, which is
 *    the sort of thing to check in the product rather than in an article.
 *  - A normal exchange (Coinbase, Kraken, Binance) cannot send to Robinhood Chain
 *    at all. Their "ETH" withdrawal lands on Ethereum mainnet, and the money is
 *    not on this chain.
 *
 * So the order is: the route that delivers the gas token first.
 *
 * Kept in step with frontend/components/WalletSheet.tsx — same two routes, same
 * warning, copy written for publishing rather than minting.
 */
...
export default function WalletSheet({
  ...
  /* Polled while open — this is the moment somebody is watching for a deposit to
     land. Stopped on close; a permanent poll buys nothing. */
  ...
  const copy = async () => {
    try {
      ...
      /* Clipboard needs a secure context and a permission that can be refused. A
         selection beats a button that lies, and on a phone it raises the native
         Copy tooltip. */
   ...
   *
   * The header cell's original job was Disconnect, and it still is for a wallet the
   * person brought with them — but a social login is routed here instead, so until
   * this row existed, signing in with Google left no way to sign out short of
   * clearing site data.
   *
  ...
  /* Portalled: the trigger lives in a sticky header, and a sticky element makes
     its own stacking context — a fixed child of it stacks at the header's level,
     not the page's. */
  return createPortal(
    <div
      ...
      >
        ...
        <div className="px-6 pt-6 pb-7">
          ...
          {/*
            ...
            390px wide — a single-line input silently clips the last third of the
            one thing this screen exists to show. `select-all` makes one tap
          ...
          */}
          ...
          {/*
            ...
            money is in Robinhood Wallet on a phone and the builder is open on a
          ...
          */}
          ...
          <div className="mt-7 border-t border-[color:var(--line)] pt-5">
            ...
            {/*
              Deliberately the ETH balance and nothing else. A republish can be
              paid in USDG, but the network fee never can — so a card showing a
              healthy USDG figure beside an empty ETH one would be reporting a
              wallet that cannot publish as a wallet that is funded.
            */}
            {empty && (
              <p className="m-0 mt-3 border border-[color:var(--line-card)] bg-[color:var(--lime)] px-3 py-2.5 text-[13px] leading-[1.5] text-[color:var(--ink)]">
                Nothing here yet. Your first site on a name is free, but the
                network fee is not — add a little ETH below and you can publish.
              </p>
            )}
          </div>
          ...
          <div className="mt-7 border-t border-[color:var(--line)] pt-5">
            ...
            <p className="m-0 mt-2 text-[14px] leading-[1.6] text-[color:var(--dim)]">
              Your first site on a name is free and rebuilds are {REBUILD_USD} in
              ETH or USDG — but every publish also pays a network fee, and that is
              always in <strong className="font-semibold">ETH</strong>. So start
              ...
              <Step n={1}>
                ...
                <div className="mt-2.5 flex flex-wrap gap-2">
                  <StoreLink
                    href={MOONPAY_IOS}
                    src="/store/appstore-badge.png"
                    alt="Download MoonPay on the App Store"
                  />
                  <StoreLink
                    href={MOONPAY_ANDROID}
                    src="/store/googleplay-badge.png"
                    alt="Get MoonPay on Google Play"
                  />
                </div>
              </Step>
              <Step n={2}>
            ...
            */}
            ...
            <p className="m-0 mt-4 text-[12px] leading-[1.55] text-[color:var(--faint)]">
              ...
              with the canonical Arbitrum bridge. A few dollars of ETH covers the
              network fees on a publish.
            </p>
          </div>
          ...
          <div className="mt-7 border-t border-[color:var(--line)] pt-5">
            ...
            {/*
              ...
              somebody hesitates over it: a wallet they did not choose, holding the
              name their site is published on, behind a button marked "sign out".
              Nothing is deleted and nothing moves — the wallet is derived from the
              account they signed in with, so the same login brings back the same
              address, and a published site is served by the name either way.
            */}
            <p className="m-0 mt-2 text-[12.5px] leading-[1.55] text-[color:var(--faint)]">
              Signing out only forgets this browser. Your names, your money and
              anything you have published stay exactly where they are — sign back
              in the same way and this wallet comes back with them.
            </p>
            {/* Ghost, not lime: this is the one thing on the card nobody should land
                on by accident, and lime is what the page uses to mean go. */}
 ...
 *
 * ALWAYS dark-on-white, in every theme — and this app has three (paper, ink,
 * lime). A QR is read by a camera, not by a person, and inverting or tinting it
 * is the fastest way to make one that scanners refuse; the quiet zone and the
 * contrast direction are part of the spec, not styling. So it sits in its own
 * white plate whatever the page is doing, with the 4-module quiet zone the spec
 * requires.
 *
 ...
 * doesn't understand the scheme shows the user a URL instead of an address. Every
 * wallet's "scan to send" understands a bare address.
 */
...
function Option({
  ...
  title: string;
  blurb: string;
  /* The provider's own mark. Both are square app icons rather than wordmarks, so
     they read at 34px and stay legible in every theme. */
...
function Step({ n, children }: { n: number; children: React.ReactNode }) {
  return (
    <li className="flex gap-3">
      ...
      {/* flex-1 as well as min-w-0: without it the content sizes to max-content and
          the store badges push the whole sheet wider than the phone. */}
...
/*
 * The official store badges, at 419x141 with transparency — the treatment both
 * stores actually ask for, and better than marks drawn by hand. Rendered at a
 * fixed height so the two sit on one baseline, and they wrap rather than squeeze
 * on a narrow phone.
 */
 ...
 *
 * Drawn rather than shipped as an asset, and only the MARK, not the lockup. The
 * lockup sets "MoonPay" in black, which disappears against this app's ink theme;
 * two circles in the brand purple read correctly on any of the three grounds and
